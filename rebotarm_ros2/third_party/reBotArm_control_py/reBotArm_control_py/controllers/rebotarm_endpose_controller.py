    ...
    def safe_home(
        ...
        for i in range(num_steps):
            if time.monotonic() > deadline:
                ...
                raise TimeoutError("safe_home trajectory send timed out")
            self._q_target[:] = traj[i]
            # Send from the homing thread as well as updating the shared target.
            # This keeps homing functional if the background control loop has
            # stopped or is temporarily unable to acquire its command lock.
            self._loop_cb(self.rebotarm, interval)
            time.sleep(interval)

        self._q_target[:] = 0.0
        self._loop_cb(self.rebotarm, dt_send)
        settle_deadline = time.monotonic() + 3.0
        max_home_error = float("inf")
        while time.monotonic() < settle_deadline:
            q_now, _, _ = self.rebotarm.get_state()
            max_home_error = float(np.max(np.abs(q_now[: self._n])))
            if max_home_error < settle_thresh:
                break
            time.sleep(self._dt)
        self._vlim_override = None
        if max_home_error >= settle_thresh:
            raise RuntimeError(
                "safe_home did not reach zero: "
                f"max joint error={max_home_error:.4f} rad "
                f"(tolerance={settle_thresh:.4f} rad)"
            )
